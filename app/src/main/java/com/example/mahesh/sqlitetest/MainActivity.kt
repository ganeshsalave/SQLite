package com.example.mahesh.sqlitetest
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        dBase.execSQL("create table if not exists employee(_id number primary key autoincrement,name varchar(100),desig varchar(100),dept varchar(100))")
// insert into table_name values(value1,'value2','value3','value4')
        insert.setOnClickListener({
            if(et1.text.toString().length>0 && et2.text.toString().length>0 && et3.text.toString().length>0 && et4.text.toString().length>0) {
                ...
                cv.put("_id", et1.text.toString().toInt())
...
//Context context, int layout, Cursor c, String[] from, int[] to, int flags
            var cAdapter = SimpleCursorAdapter(this@MainActivity,
                    R.layout.indiview,c, arrayOf<String>("_id","name","desig","dept"),
